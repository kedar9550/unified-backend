...
async function run() {
  ...
  for (const c of testChapters) {
    console.log(`\n=================== TITLE ONLY SEARCH: "${c.chapterTitle}" ===================`);
    // Search with TITLE() instead of TITLE-ABS-KEY()
    const searchUrl = `https://api.elsevier.com/content/search/scopus?query=TITLE("${encodeURIComponent(c.chapterTitle)}")&count=5`;
